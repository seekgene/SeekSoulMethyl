...
pub enum BarcodeState {
    ...
    OneMatch(MutateSeq), // Match with 1 mismatch
    MultiMatch(Vec<MutateSeq>), // Match with multiple mismatches
    NoMatch, // No match
}
...
// Define a struct for storing Read state
#[derive(Debug)]
struct ReadStat{
    // Store sequence-related statistics, using 2D array format, with specific types and dimensions determined at initialization
    seq_stat: Array<usize, Dim<[usize; 2]>>,
    // Store quality-related statistics, also using 2D array format, allowing different dimensions from sequence statistics
    qual_stat: Array<usize, Dim<[usize; 2]>>,
}

// Implement ReadStat struct operations
impl ReadStat {
    /// Create a new ReadStat instance.
    ///
    /// Initialize sequence statistics (seq_stat) and quality statistics (qual_stat) arrays for subsequent statistical operations.
    fn new() -> Self {
    ...
    /// Update statistical information of a ReadStat instance.
    ///
    /// Add sequence statistics (seq_stat) and quality statistics (qual_stat) of two ReadStat instances,
    /// storing the result in the first instance (a).
    fn update(a: &mut ReadStat, b:ReadStat){
        a.seq_stat = &a.seq_stat + b.seq_stat;
        a.qual_stat = &a.qual_stat + b.qual_stat;
    }
}
/// `Chunked` struct for chunking iterators.
/// 
/// Chunking is an operation that splits large amounts of data into smaller, more manageable chunks,
/// which is very useful for processing large datasets, such as reading large files or data in network transmission.
/// 
/// # Parameters
/// - `I`: A type that can be converted to an iterator. This allows `Chunked` to handle various iterable data structures.
/// - `chunk_size`: The size of chunks, determining the number of elements each chunk contains. If a chunk has fewer elements than `chunk_size`,
///   then this is the last chunk, and there will be no more chunks after it.
/// 
/// # Example
/// 
/// ```rust
...
/// Create a chunked iterator that splits a collection into chunks of specified size
///
/// # Parameters
/// - `c`: A collection that can be converted to an iterator
/// - `chunk_size`: The size of each chunk
///
/// # Returns
/// Returns a `Chunked` struct containing the original iterator and chunk size
///
/// # Type Parameters
/// `Collection`: A collection type that implements the `IntoIterator` trait
///
/// # Type Constraints
/// - `Collection: IntoIterator`: The collection type must be convertible to an iterator
///
/// # Example
/// ```rust
/// let numbers = vec![1, 2, 3, 4, 5, 6, 7, 8];
/// let chunks = chunked(numbers, 3);
/// // chunks is now a chunked iterator with chunk size of 3
/// ```
    ...
    Collection: IntoIterator,
{
    ...
    // Create and return a Chunked struct, setting the iterator and chunk size
    Chunked {
        iterator,
        chunk_size,
    }
...
/// Implement chunking functionality for iterators
///
/// # Generic Parameters
/// - `I`: A type that implements the iterator trait
///
/// # Associated Types
/// - `Item`: A type that wraps a vector of the original iterator's element type
///
/// # Methods
/// - `next`: Returns an Option type containing a vector with at most `chunk_size` elements
///   - This method creates chunks by fetching elements on-demand from the underlying iterator
///   - Each call fetches at most `chunk_size` elements from the iterator and collects them into a vector
///   - If the fetched elements form an empty vector, it is filtered out and not returned
...
/// Convert base to corresponding number
///
/// # Parameters
/// - `base`: A reference to a u8 type representing a base
///
/// # Return Value
/// - `Some(u8)`: Represents the successfully converted number, ranging from 0 to 4, corresponding to different bases
/// - `None`: Returns None if the input base cannot be recognized
///
/// # Description
/// This function is used to convert base symbols in DNA/RNA to numeric codes for convenient computer processing and storage
/// Specifically, 'T' and 'U' are treated as the same base, because in RNA, 'T' is replaced by 'U'
/// 'N' represents any unknown or unspecified base, while None is used to represent unrecognized input
fn base_to_number(base: &u8) -> Option<u8> {
    match base {
        &b'A' => Some(0),
        &b'T' | &b'U' => Some(1), // U represents Uracil, used in RNA
        &b'C' => Some(2),
        &b'G' => Some(3),
        &b'N' => Some(4), // N represents unknown or any base
        _ => None,       // If not one of the above bases, return None
    }
}
...
/// Write sequence statistics and quality statistics to CSV files
/// 
/// # Parameters
/// - `stat`: Borrowed read statistics containing sequence and quality statistics
/// - `out_prefix`: Output filename prefix used to generate output filenames
fn write_stat(stat: &ReadStat, out_prefix: String) {
    // Construct sequence statistics filename and create corresponding CSV writer
    let seq_stat_file = out_prefix.clone() + "base_stat.csv";
    let mut wtr0 = WriterBuilder::new().from_path(seq_stat_file).unwrap();
    // Write sequence statistics header
    wtr0.write_record(&["pos", "A", "T", "C", "G", "N"]).unwrap();
    
    // Find the first row index with non-zero values in sequence statistics
    ...
    // Define column index and slice write sequence statistics
    let col_idx: usize = 4;
    ...
    // Construct quality statistics filename and create corresponding CSV writer
    let qual_stat_file = out_prefix.clone() + "qual_stat.csv";
    let mut wtr1 = WriterBuilder::new().from_path(qual_stat_file).unwrap();
    
    // Find the first row and column index with non-zero values in quality statistics
    let mut row_idx: usize = 0;
    ...
    // Write quality statistics header
    ...
    // Slice and write quality statistics
