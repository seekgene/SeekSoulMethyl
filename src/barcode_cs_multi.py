...
class Reader(Process):
    ...
    def __init__(self, file1:str, file2:str, connections:multiprocessing.connection, queue:Queue, buffer_size:int):
        ...
        Args:
            ...
            buffer_size: Buffer size when reading fastq files
...
class Writer:
    ...
    def __init__(self, file:str, file_multi:str, paired_out:bool=False):
        ...
        Args:
            ...
            file_muti: Fastq file name for sequences with undetermined barcodes
        """
...
class AdapterFilter:
    ...
    def __init__(self, adapter1:list=[], adapter2:list=[],):
        ...
        # Support error tolerance rate and overlap parameters
        # sequence: str,
        # max_errors: float = 0.1,
        # min_overlap: int = 16,
        for p in adapter1:
            if len(p) >= 4:  # If there are additional parameters (error tolerance rate and overlap)
                if p[1] == "3":
    ...
    def filter(self, r1=None, r2=None) -> tuple:
        ...
        if r1 and self.adapter1:
            sp2_target = False  # Flag indicating whether target adapter is detected
...
def parse_structure(string:str) -> tuple:
    ...
    Use letters B, L, U, X and T along with numbers to represent read structure.
    B represents barcode base positions;
    L represents linker base positions;
    U represents UMI base positions;
    X represents arbitrary bases for placeholder;
    T represents T base;
    ...
    Returns:
        Returns a 2D tuple containing the structure and length of each part in order.
        For example:
...
def get_new_bc(bc:str, white_list:set, distance:int)->set:
    """Return the intersection of the set of error-corrected barcodes (allowing for a defined number of mismatches) at each position and the whitelist set."""
                    ...
                    if l17me == 'GTAGATGTGTATAAGAGA':
                        ...
                        if average_quality >= 30:
                            if insert == 'CC': 
                                ...
                                if A_count_T + A_count_C == len(countseq): # Must be C or T, if other bases indicate sequencing error
                                    stat_Dict["line_A"] += 1
