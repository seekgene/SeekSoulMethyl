...
def umi_correct(geneid_umi_dict, bc, umi_correct_detail_fh):
    ...
    for gene_id, umi_dict in geneid_umi_dict.items():
        # Direct counting without UMI correction
        counts_dict[gene_id][0] = len(umi_dict.keys())  # UMI count
        counts_dict[gene_id][1] = sum(umi_dict.values())  # Total reads count
    ...
    for barcode_umi, g in groupby(reads_group, key=lambda x: x.qname):
        ...
        for r in g:
            ...
            if r_id in reads_idlist:
                ...
                # Use alignment position as gene_id and umi
