...
def totalcpg(allcpgfile):
    df = pd.read_csv(allcpgfile, 
                     sep='\t',
                     usecols=[3],  # Only read the 4th column
...
def main(cbfile, outdir, samplename, outcsv, allcpgfile, indir, summary_json):
    ...
    # Sort by cov1 column from high to low
    ...
    # Output the umi column value and reads column value of the cb with the highest cov1
    ...
    # Output the cb, umi and reads column values at the corresponding index position
