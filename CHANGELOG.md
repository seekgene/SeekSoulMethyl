...
- `nf/nextflow.config`: Bumped manifest `nextflowVersion` from `>=21.04.0` to `>=24.10.0` to match the actual runtime requirement of the `oomAwareMemory` closure (`task.previousTrace`). Without this, Nextflow 21.04–24.09 pass the manifest check but crash on the first dynamic-memory retry.

### Fixed
- **`barcode_cs_multi.py`: Multi-read chain direction was re-determined from the post-adapter-trim R1 sequence in the multi rescue second pass, which could flip `forward`/`reverse` relative to the pre-trim direction used in the first pass or return `unknown` when the trimmed R1 was too short to cover the direction-diagnostic positions.** In v2.1.2 and earlier, `unknown` multi reads were counted as `valid` but were not counted as `forward`/`reverse`, were not included in `forward_chimeric`/`reverse_chimeric`, and were not written to Bismark input FASTQ files. This silently dropped rescued multi reads and made `Dropped_Chimeric` artificially low because the `unknown` reads inflated the denominator without contributing to the chimeric numerator. Fixed by carrying the pre-trim `chain_direction` through the multi temp FASTQ read name (`barcode_old_candidate_list_umi_chain_direction_original_read_name`) and reusing it in the second pass.
...
- `step3_mcds_manager.py`: Added a defensive pre-merge guard `_validate_unique_obs_across_shards()`. `xarray.Dataset.to_zarr(append_dim=...)` does not reject duplicate `cell` coordinates, so a shard merge with duplicate cells (within or across shards) would silently produce a merged MCDS whose `cell` index contains repeated entries instead of failing. The new check fails fast with an explicit error if duplicates are detected. No duplicate cells were observed in tiny-demo data; this hardens the merge against the theoretical case.
- `step4_report_rna_met.py`: Added a defensive `deduplicate_cells_by_m_cb()` that aggregates shard-level methylation cell metrics to one row per `m_cb` (additive counters via `sum`, recomputing `weighted_mc_rate` / `cell_saturation` / `*_mc_rate` from summed counters, and validating `reads_counts` exists and is numeric) before the RNA+methylation join. Without it, duplicate `m_cb` rows would propagate into the join as duplicated/cartesian rows. No duplicate `m_cb` rows were observed in tiny-demo data; this guards the join against the theoretical case.
